...
CAKRA_REPO="punggawacybersecurity/cakra-agent"
...
configEDR() {
  ...
  downloadFile "https://raw.githubusercontent.com/${CAKRA_REPO}/latest/conf/local_internal_options.conf" "/var/ossec/etc/local_internal_options.conf" >> "$LOG_PATH" 2>&1
  chown -R root:wazuh /var/ossec/etc/local_internal_options.conf >> "$LOG_PATH" 2>&1
  downloadFile "https://raw.githubusercontent.com/${CAKRA_REPO}/latest/conf/open-audit.sh" "$BASE_PATH/scripts/open-audit.sh" >> "$LOG_PATH" 2>&1
...
configHIDS() {
  ...
  downloadFile "https://raw.githubusercontent.com/${CAKRA_REPO}/latest/conf/osquery.conf" "/etc/osquery/osquery.conf" >> "$LOG_PATH" 2>&1
  downloadFile "https://raw.githubusercontent.com/${CAKRA_REPO}/latest/conf/osquery.flags" "/etc/osquery/osquery.flags" >> "$LOG_PATH" 2>&1
...
configNIDS() {
  ...
  downloadFile "https://raw.githubusercontent.com/${CAKRA_REPO}/latest/conf/packetbeat.yaml" "/etc/packetbeat/packetbeat.yml" >> "$LOG_PATH" 2>&1
}
...
checkDFIRConnection() {
  downloadFile "https://raw.githubusercontent.com/${CAKRA_REPO}/latest/conf/dfir/${KEY}.yaml" "${BASE_PATH}/config/${KEY}.yaml" >> "$LOG_PATH" 2>&1
